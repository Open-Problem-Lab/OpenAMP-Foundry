...
> **New in v0.5.58:** `make bench-simulation-baselines` compares shipped simulation modules against their cheap baselines. Membrane beats Boman on AMP-vs-decoy (`0.7512` vs `0.3899`) and within-AMP selectivity ratio (`0.6385` vs `0.4834`), but structure does not beat helicity baseline (`0.6458` vs `0.6486`) and rich_selectivity remains stronger for hemolysis detection (`0.7453`). Weighted mode remains blocked. Full suite: 1941 passed, 7 skipped.
...
> **New in v0.5.51:** Virtual assay scope document — `docs/VIRTUAL_ASSAY_SCOPE.md` with uncertainty policy, ablation requirements, integration modes. Doc drift fixes: BENCHMARKING.md, METRICS_CURRENT.md test count, NOVELTY_AUDIT_GUIDE.md script ref. No code changes.

...
| 38 ✅ | No API contract for third-party simulation modules (e.g. Martini MD, AlphaFold) | `simulation/interfaces.py` extended with `ExternalSimulationAdapter` protocol and `validate_external_adapter_result()`. Documented in ARCHITECTURE.md extension points. v0.5.59. | Third-party adapters can implement the protocol without reading pipeline internals; validation rejects length, module, version, and uncertainty mismatches. |
...
**Next loop:** Loop 39 — simulation benchmark report against strict triage.

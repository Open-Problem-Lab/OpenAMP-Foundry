...
> **Status:** Strategic roadmap. Updated v0.5.36.
> **Current state:** 1762 tests, pipeline AUROC 0.7792, calibration intake + gate + engine shipped,
> Phase 0 complete (Loops 1–8), Phase 1 complete (Loops 9–17), Phase 2 Loops 18, 21 complete,
...
| 19 ✅ | No policy version tracking. Every recalibration policy edit needs a version bump, locked-changes enforcement, and a decision-log entry | `calibration/policy_version.py`: compares current vs previous policy files, validates prior `locked_changes` are unchanged, and rejects substantive edits without a policy-version bump plus non-empty decision log dated within 30 days. v0.5.40 | CLI `policy-version-check`; 7 focused tests; full suite passing |
...
Phase 2: Loops 18, 19, 21 complete (per-family benchmark, policy version guard, recalibration engine)
...
|------|-----------|-------------|-------------|
| 18 ✅ | No per-family benchmark breakdown. The 500-AMP benchmark treats all AMPs as one class, hiding which families the pipeline handles well or poorly | `scripts/benchmark_per_family.py`: stratifies 500-AMP set by 6 heuristic structural classes. Reports per-class AUROC. v0.5.37. **Key finding:** Pipeline is charge-dominated — highly_cationic AUROC 0.958 vs proline_rich 0.586 (Δ=0.37). Weak classes flagged as blind spots for wet-lab selection. | Per-family AUROC table in METRICS_CURRENT.md. Families below 0.70 flagged. 27 tests, 1762 passing. |
| 19 ✅ | No policy version tracking. Every recalibration policy edit needs a version bump, locked-change preservation, and a decision-log entry | `calibration/policy_version.py` compares current vs previous policy files and rejects silent substantive drift. v0.5.40 | CLI `policy-version-check`; 7 focused tests |
...
**Next loop:** Loop 20 — Phase 2 (synthetic lab-result generator).

...
def validate_policy_version_update(
    ...
    *,
    ...
    A no-op policy is allowed without a decision log. Any substantive policy
    change requires:

    - current ``policy_version`` > previous ``policy_version``;
    - every previously locked rule still present with identical lock metadata;
    - a non-empty ``docs/DECISION_LOG_YYYY-MM-DD.md`` dated within 30 days.
    """

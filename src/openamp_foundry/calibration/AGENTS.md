...
```mermaid
flowchart LR
  Intake["intake.py"] --> Gate["recalibration_gate.py"]
  Policy["policy.py"] --> Gate
  Gate --> Engine["engine.py"]
  PolicyVersion["policy_version.py"] --> Policy
  PolicyVersion --> DecisionLog["docs/DECISION_LOG_YYYY-MM-DD.md"]
```

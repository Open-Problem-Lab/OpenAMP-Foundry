...
class TestDocsConsistent:
    ...
    def test_doc_metrics_snapshot_includes_ranking_policy(self):
        ...
        assert (
            payload["charge_balanced_synthetic_decoys"]["benchmark"]
            == "charge_balanced_synthetic_decoys"
        )
        assert payload["charge_balanced_synthetic_decoys"]["n_positives"] == 500
        assert payload["charge_balanced_synthetic_decoys"]["charge_density_auroc"] == 0.5

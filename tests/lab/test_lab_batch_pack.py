...
def test_build_pack_success(tmp_path):
    ...
    assert result["strict_identity_checks"]["evidence_must_match_panel_exactly"] is True
...
def test_build_pack_contains_real_review_reference_docs(tmp_path):
    ...
    ev_dir = _make_evidence(tmp_path, "C1", "C2")
    out = tmp_path / "pack.zip"
    result = build_batch_pack(panel_csv=str(panel), evidence_dir=str(ev_dir), out_zip=str(out))
    assert "error" not in result
    with zipfile.ZipFile(out) as z:
        names = z.namelist()
